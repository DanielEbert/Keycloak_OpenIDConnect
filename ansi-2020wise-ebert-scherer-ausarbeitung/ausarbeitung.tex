...
% Er ist optimiert für vertrauenswürdige Clients \cite{RFC6749}. 
...
Diese Gegenmaßnahme wird durch Keycloak unterstützt. Ein Authorization Code kann nur einmal verwendet werden und als zusätzliche Maßnahme ist die Lebensdauer für Authorization Codes sehr kurz - einige Sekunden, sodass der Benutzer seinen Authorization Code gegen ein Access Token eintauschen kann \cite[Sec. 19]{keycloakDocs}.
...
In Abbildung \ref{fig:EB_Struktur des Git-Verzeichnisses} ist die Struktur des Git-Verzeichnisses zu sehen, welches wir für dieses Beispiel erstellt haben. Dieses Git Repository kann unter TODO geclont werden. Um das Beispiel zu starten, muss Docker, Docker-Compose und das Git Repository TODO heruntergeladen bzw. installiert sein. Außerdem muss, wie in der \texttt{src/README.md} näher beschrieben, der Pfad zum \texttt{postgres\_data} Ordner in der \texttt{src/.env}-Datei gesetzt werden.

%TODO will der vmware oder virtualbox?
Als Alternative zur manuellen Installation des letzten Absatzes kann die von uns erstelle VMWare VM unter TODO verwendet werden. Die Benutzernamen und Passwörter für die VM und die Keycloak Benutzer sind in der src/README.md Datei des Git Repositories TODO.
